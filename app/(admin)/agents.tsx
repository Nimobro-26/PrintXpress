...
export default function AdminAgentsScreen() {
  ...
  const handleRemove = (agent: typeof AGENTS[0]) => {
    ...
    if (Platform.OS === 'web') {
      ...
      if (ok) alert(`${agent.name} has been removed from the fleet.`);
    } else {
      Alert.alert('Remove Agent', msg, [
        { text: 'Remove', style: 'destructive', onPress: () => Alert.alert('Removed', `${agent.name} has been removed.`) },
        { text: 'Cancel', style: 'cancel' },
      ]);
    }
  };
  ...
  const handleAddAgent = () => {
    if (Platform.OS === 'web') {
      ...
      if (!zone) return;
      alert(`✓ Agent Added\n\nName: ${name}\nPhone: ${phone}\nZone: ${zone}\n\nAn invitation SMS has been sent to the agent.`);
    } else {
      Alert.alert('Add New Agent', 'Choose how to register:', [
        {
          text: 'Enter Details',
          onPress: () =>
            Alert.alert(
        ...
        },
        {
          ...
          onPress: () => Alert.alert('Invite Sent', 'An SMS invite link has been sent. The agent can complete registration via the link.'),
        },
        { text: 'Cancel', style: 'cancel' },
      ]);
    }
...
const styles = StyleSheet.create({
  ...
  filtersContent: { paddingHorizontal: 20, gap: 8, paddingRight: 20 },
  filterChip: {
